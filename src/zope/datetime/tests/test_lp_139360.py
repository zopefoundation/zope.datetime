...
DD = [(d, dr.rjust(2, '0')) for d, dr in D] + [(d, repr(d))
                                               for d in range(10, 31)]
MM = [(m, mr.rjust(2, '0')) for m, mr in M] + [(m, repr(m))
                                               for m in range(10, 13)]

# The set of years:
YYYY = [(2001, '2001')]

# The sets of mixed-endian date strings (where each string is stored with
# its component values):
M_D_YYYY = [((d, m, y), s.join([mr, dr, yr]))
            for d, dr in D for m, mr in M for y, yr in YYYY]
M_DD_YYYY = [((d, m, y), s.join([mr, dr, yr]))
             for d, dr in DD for m, mr in M for y, yr in YYYY]
MM_D_YYYY = [((d, m, y), s.join([mr, dr, yr]))
             for d, dr in D for m, mr in MM for y, yr in YYYY]
MM_DD_YYYY = [((d, m, y), s.join([mr, dr, yr]))
              for d, dr in DD for m, mr in MM for y, yr in YYYY]

# The sets of big-endian date strings (where each string is stored with
# its component values):
YYYY_M_D = [((d, m, y), s.join([yr, mr, dr]))
            for d, dr in D for m, mr in M for y, yr in YYYY]
YYYY_M_DD = [((d, m, y), s.join([yr, mr, dr]))
             for d, dr in DD for m, mr in M for y, yr in YYYY]
YYYY_MM_D = [((d, m, y), s.join([yr, mr, dr]))
             for d, dr in D for m, mr in MM for y, yr in YYYY]
YYYY_MM_DD = [((d, m, y), s.join([yr, mr, dr]))
              for d, dr in DD for m, mr in MM for y, yr in YYYY]
